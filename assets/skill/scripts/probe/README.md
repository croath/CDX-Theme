...
| `go-work-home.js` | Click “New task” / equivalent locale labels to open Work home |
...
| `sidebar-ink.js` | Left rail (`.app-shell-left-panel` aside **and** Settings `div`): whiteish labels, `.sidebar-foreground-muted` remap | White / invisible sidebar text |
| `contrast-scan.js` | Full viewport: whiteish direct text + low-luma solid backgrounds | Broad hard-to-read UI / mixed light+dark leftovers |
...
- Localized host labels are covered in probe scripts where relevant (EN + common locale `aria-label`s).
